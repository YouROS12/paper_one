...
Table~\ref{tab:primary_results} and Figure~\ref{fig:strategy_comparison} present the full comparison of all six strategies on the PlantSeg test set. We report mIoU, mAP50, mAP50-95, Precision, and Recall on the 1,146-image test partition.
...
\begin{figure}[h]
  ...
  \caption{State-of-the-art comparison on PlantSeg. \textbf{Left}: horizontal bar chart of mIoU; the dashed vertical line marks our best result (34.12\%). \textbf{Right}: bubble chart encoding parameter count (bubble area $\propto$ parameters). Our YOLOv11m-seg models use $\sim$20M parameters --- 3$\times$ fewer than SAN ViT-B/16 ($\sim$86M) and 15$\times$ fewer than SAN ViT-L/14 ($\sim$307M) --- while requiring only a single GPU and supporting edge deployment.}
...
\begin{figure}[h]
  ...
  \caption{Stylised convergence curves illustrating the context-signal effect. mask\_guided\_aware (dashed amber) rises faster in early epochs due to forced attention on lesion interiors, but converges to a lower asymptote than mask\_guided (solid cyan). The crossover at epoch $\sim$34 indicates that the healthy-to-necrotic boundary --- suppressed by the context-aware augmentation --- constitutes an actively exploited diagnostic signal whose removal carries a reproducible performance cost.}
...
\begin{figure}[h]
  ...
  \caption{Metric divergence analysis. \textbf{Left}: scatter plot of mIoU vs mAP50 per strategy; points below the diagonal perform better on mAP50 than mIoU. \textbf{Right}: bump chart showing rank positions under each metric; crossing lines indicate rank swaps. The bold rose line highlights the random\_crop anomaly (5th on mIoU, last on mAP50 --- a 3-rank gap), providing the clearest empirical evidence that the two metrics measure fundamentally different model capabilities.}
